...
- The Edamam api receives a list of ingredients, but requires a certain "quality" of ingredients. Firstly, when adding the ingredients cheese or bread, water needs to also be added as an ingredient. Secondly, when entering ingredients you CAN submit items without a quantity, but it is better to assign ingredients a quantity if you have it. The Edamam api will try to assign a quantity to them based on the expected serving size. Thirdly, the api will not use raw ingredients. And fourthly, the api can use single and two part compounds (“chicken” or “rice AND chicken”). Any issues with the "quality" of the recipe will be represented by a 555 error. Other possible reasons you might get an error are: too high or too low weight of a serving, unrecognized ingredients or ingredients to which no weight can be assigned. 
- In our experience with the api we found that you cannot simply add a few ingredients like salt, and pepper, but you have to make a list of ingredients that represent a more full recipe, as opposed to a few ingredients.
